// Phase 0 baseline — faithful port of the target's /tic-tac-cheat/app.js.
// Wires the reconstructed shell so it plays exactly like the original.
// Later phases layer the tiered engine, composure, counter-measures, the
// tribunal, and the global counter on top of this loop.
...
async function play(index) {
  ...
  if (winner(board, 'O')) {
    ...
    $('#status').textContent = incidents
      ? ['A triumph of talent. And editing.', 'I would like to thank the referee. Me.', 'A fair result, according to me.'][Math.floor(Math.random() * 3)]
      : 'A perfectly ordinary victory. Suspicious.';
    setTurn('');
